...
def _patched_eval_policy(task_name, TASK_ENV, args, model, st_seed,
    ...
    def _logging_setup_demo(now_ep_num=None, seed=None, is_test=True, **kw):
        ...
        if _episode_timer["start"] is not None and len(_episode_log["infer_calls"]) > 0:
            _finalize_episode(TASK_ENV, _episode_timer, _step_counter, task_name)
...
def _finalize_episode(TASK_ENV, timer, step_counter, task_name):
    """Collect all data for one completed episode."""
    elapsed_s = time.monotonic() - timer["start"]

    infer_times = [c["client_time_ms"] for c in _episode_log["infer_calls"]]
    server_infer_times = [c["server_infer_ms"] for c in _episode_log["infer_calls"]]
    kv_times = [c["client_time_ms"] for c in _episode_log["kv_cache_calls"]]
    ...
    episode_record = {
        ...
        # Inference timing
        "num_infer_chunks": len(_episode_log["infer_calls"]),
        "num_kv_cache_calls": len(_episode_log["kv_cache_calls"]),
        "reset_time_ms": round(_episode_log["reset_time_ms"], 2) if _episode_log["reset_time_ms"] else None,
        "total_client_infer_time_s": round(sum(infer_times) / 1000, 3),
        "total_server_infer_time_s": round(sum(server_infer_times) / 1000, 3),
        "total_kv_cache_time_s": round(sum(kv_times) / 1000, 3),
        "avg_chunk_infer_ms": round(sum(infer_times) / max(len(infer_times), 1), 2),
        "avg_server_infer_ms": round(sum(server_infer_times) / max(len(server_infer_times), 1), 2),
        # Detailed per-call breakdown
        "infer_calls_detail": _episode_log["infer_calls"],
        "kv_cache_calls_detail": _episode_log["kv_cache_calls"],
    }
    ...
    print(
        ...
        f"infer={episode_record['total_client_infer_time_s']}s\033[0m"
    )
...
def _save_csv_summary(episodes, csv_path):
    ...
    fields = [
        ...
        "total_client_infer_time_s", "total_server_infer_time_s",
        "total_kv_cache_time_s", "avg_chunk_infer_ms", "avg_server_infer_ms",
    ]
