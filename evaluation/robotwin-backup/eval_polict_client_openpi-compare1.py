...
def save_comparison_video(real_obs_list, imagined_video, action_history, save_path, fps=10):
    ...
    final_frames = []
    fixed_frame_size = None  # Ensure all frames have the same size
    ...
    for i in range(n_frames):
        ...
        # Use imagined frame if available; repeat last frame if exhausted
        img_idx = min(i, n_imagined - 1) if n_imagined > 0 else -1

        if imagined_video is not None and img_idx >= 0:
            img_frame = imagined_video[img_idx]
            ...
            # Split T-shape frame into 3 individual camera views
            im_high, im_left, im_right = _split_tshape_frame(img_frame)

            # Arrange as High | Left | Right, same layout as real observation
            row_imagined = np.hstack([
                resize_h(im_high, base_h),
                resize_h(im_left, base_h),
                resize_h(im_right, base_h)
            ])
            # Ensure width matches real observation row
        ...
        row_imagined = add_title_bar(row_imagined, "Imagined Video Stream (High / Left / Right)")
        full_frame = np.vstack([row_real, row_imagined])
        full_frame = np.ascontiguousarray(full_frame)

        # Ensure all frames have the same size (fix imageio crash)
