#!/bin/bash
# launch_client_multigpus_with_logging.sh
# Multi-GPU version of launch_client_with_logging.sh
# Each task runs in a separate background process with detailed logging.
#
# Usage:
#   bash evaluation/robotwin/launch_client_multigpus_with_logging.sh <save_root> <task_list_id> [seed] [test_num]
#
# Examples:
#   bash evaluation/robotwin/launch_client_multigpus_with_logging.sh ./results 0          # Group 0, 8 tasks on 8 GPUs
#   bash evaluation/robotwin/launch_client_multigpus_with_logging.sh ./results 1 0 50     # Group 1, seed=0, 50 episodes each
#
# Output:
#   - Same eval results as the original script
#   - PLUS detailed_log.json and episode_summary.csv per task under:
#     <save_root>/stseed-<st_seed>/detailed_logs/<task_name>/
...
echo -e "\033[32mLaunching ${#task_names[@]} tasks WITH LOGGING. GPUs assigned by mod ${num_gpus}, ports starting from ${start_port}.\033[0m"
...
for i in "${!task_names[@]}"; do
    ...
    XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 python -m evaluation.robotwin.eval_with_logging --config policy/$policy_name/deploy_policy.yml \
        --overrides \
        --task_name ${task_name} \
...
echo -e "\033[32mAll tasks launched with logging. PIDs saved to ${pid_file}\033[0m"
echo -e "\033[36mTo terminate all processes, run: kill \$(cat ${pid_file})\033[0m"
echo -e "\033[36mDetailed logs will be saved under: ${save_root}/stseed-*/detailed_logs/\033[0m"
